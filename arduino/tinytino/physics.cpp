...
Body* World::getBodyById(int id) {
    for (int i=0;i<bodyCount; i+=1) {
        if (bodyList[i].id == id) return &bodyList[i];
    }
    return 0;
}
...
void Body::updateStep(Camera& camera) {
    ...
    velocity = velocity *  flyingDrag + Fixed2D4(0,0,0,4);
    position+= velocity;
    ...
    if ((posX != oldPosX || posY != oldPosY) && (spriteW > 0 && spriteH > 0)) {
        ...
        if (hit) {
            ...
            velocity = -velocity * contactDrag - (before - position) * bounceFactor;
        }

    }
    if (bodyHandler != 0) {
        bodyHandler->handle(this);
    }
